...
const chat = () => {
  ...
  const buscarChats = (e) => {
    ...
    if (campoBusqueda) {
      ...
      const buscar = chats.filter((chat) => chat.nombreContacto.toUpperCase().includes(campoBusqueda.toUpperCase()) || chat.apellidoContacto.toUpperCase().includes(campoBusqueda.toUpperCase()));
      console.log(buscar);
  ...
  function ago(v){v=0|(Date.now()-v)/1e3;var a,b={segundo:60,minuto:60,hora:24,día:7,semana:4.35,mes:12,año:1e4},c;for(a in b){c=v%b[a];if(!(v=0|v/b[a]))return c+' '+(c-1?a+'s':a)}}
                ...
                <div className="list-group rounded-0">
                  {
                    (campoBusqueda)?
                      ...
                      return <a
                        key={chat.idChat}
                        href={"#"+chat.mensajes[chat.mensajes.length-1].idMensaje}
                        onClick={()=>{handleClicChat(chat.idChat)}}
                        className={chat.idChat==chatActual.idChat?
                        ...
                        <div className="media">
                          ...
                          <div className="media-body ml-4">
                            <div className="d-flex align-items-center justify-content-between mb-1">
                              <h6 className="mb-0">{chat.nombreContacto} {chat.apellidoContacto}</h6>
                              <small className="small font-weight-bold">
                                {chat.mensajes[0]?ago(new Date(chat.mensajes[chat.mensajes.length-1].fecha).getTime()):<></>}
                              </small>
                            </div>
                            <p className="font-italic text-muted mb-0 text-small">
                              {
                                chat.mensajes[0]?
                                (chat.mensajes[chat.mensajes.length-1].mensaje.substr(0, 20) + 
                                (chat.mensajes[chat.mensajes.length-1].mensaje.length>20?"...":
                                "")):
                                ""}
                            </p>
                          </div>
                    ...
                    return <a
                      key={chat.idChat}
                      href={(chat.mensajes)?((chat.mensajes[0])?"#"+chat.mensajes[chat.mensajes.length-1].idMensaje:""):""}
                      onClick={()=>{handleClicChat(chat.idChat)}}
                      className={chat.idChat==chatActual.idChat?
                      "list-group-item list-group-item-action active text-white rounded-0":
                      "list-group-item list-group-item-action list-group-item-light rounded-0"}
                      ...
                      <div className="media">
                        ...
                        <div className="media-body ml-4">
                          ...
                          <p className="font-italic text-muted mb-0 text-small">
                            {
                              chat.mensajes[0]?(
                                (chat.mensajes[chat.mensajes.length-1].tipo=='venta')?
                                "Venta":
                                (
                                  (chat.mensajes[chat.mensajes.length-1].mensaje.substr(0, 20) + 
                                  (chat.mensajes[chat.mensajes.length-1].mensaje.length>20?"...":
          ...
          <div className="col-7 px-0">
            <div className="bg-gray px-4 py-2 bg-light hola">
              <div className="col-sm-3 col-xs-3 heading-avatar">
                ...
                />
                <div class="col-sm-8 col-xs-7 heading-name">
                  <a class="heading-name-meta">{chatActual.nombreContacto} {chatActual.apellidoContacto}</a>
                ...
                chatActual.mensajes.map((mensaje)=> {
                  if (mensaje.idUsuario!=user.idUsuario) {
                    ...
                    if (mensaje.tipo == "texto") {
                      return <div className="media w-50 mb-3" key={mensaje.idMensaje} id={mensaje.idMensaje}>
                      <img
                        src="https://bootstrapious.com/i/snippets/sn-chat/avatar.svg"
                        alt="user"
                        width="50"
                      ...
                      <div className="media-body ml-3">
                        ...
                        <p className="small text-muted">{ago(new Date(mensaje.fecha).getTime())}</p>
                      </div>
                    </div>
                    } else if (mensaje.tipo == "venta") {
                      return <div class="antialiased bg-gray-200 font-sans" key={mensaje.idMensaje} id={mensaje.idMensaje}>
                            ...
                            <div class="max-w-md md:max-w-2xl px-2">
                                <div class="bg-white shadow-xl rounded-lg overflow-hidden md:flex">
                                    <div class="bg-cover bg-bottom h-56 md:h-auto md:w-56" >
                                      <img src={"http://localhost:4000/uploads/"+ JSON.parse(mensaje.mensaje).foto} width={200} height={200}/>
                                    </div>
                                    <div>
                                        <div class="p-4 md:p-5">
                                            <p class="font-bold text-xl md:text-2xl">{JSON.parse(mensaje.mensaje).producto}</p>
                                            <p class="text-gray-700 md:text-lg">{JSON.parse(mensaje.mensaje).descripcion}</p>
                                        </div>
                                        <div class="p-4 md:p-5 bg-gray-100">
                                          <button class="mt-3 sm:mt-0 py-2 px-5 md:py-3 md:px-6 bg-indigo-700 hover:bg-indigo-600 font-bold text-white md:text-lg rounded-lg shadow-md" onClick={()=>{handleVer(JSON.parse(mensaje.mensaje).idVenta)}}>ver mas</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    if (mensaje.tipo == "texto") {
                      return <div className="media w-50 ml-auto mb-3" key={mensaje.idMensaje} id={mensaje.idMensaje}>
